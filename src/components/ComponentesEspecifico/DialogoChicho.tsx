  ...
  export default function DialogoChicho({ motivation }: { motivation?: string }) {
    ...
    useEffect(() => {
      async function getDialogos() {
        const res = motivacion === null 
          ? await supabase.from('Mensajes').select().is('tipoMotivacion', motivacion) 
          : await supabase.from('Mensajes').select().eq('tipoMotivacion', motivacion)
